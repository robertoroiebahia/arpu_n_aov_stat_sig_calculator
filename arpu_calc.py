...
def input_revenues(group_name):
    ...
    """
    total_users = int(input(f"Enter total number of users for {group_name}: "))
    revenue_input = input(f"Enter revenue values for {group_name} (comma-separated for users who purchased): ")
...
if __name__ == '__main__':
    # Input data for Group A (Control)
    ...
    # AOV is calculated from purchasers only
    if len(purchaser_revenues_A) > 0:
    ...
    # Input data for Group B (Variant)
    ...
    # Display ARPU and AOV metrics
    print(f"\nGroup A (Control): n = {n_A}, Total Revenue = {total_revenue_A:.2f}, ARPU = {arpu_A:.2f}, ARPU SD = {sd_arpu_A:.2f}")
    print(f"Group A: Purchasers = {n_orders_A}, AOV = {aov_A:.2f}, AOV SD = {sd_aov_A:.2f}")
    
    print(f"\nGroup B (Variant): n = {n_B}, Total Revenue = {total_revenue_B:.2f}, ARPU = {arpu_B:.2f}, ARPU SD = {sd_arpu_B:.2f}")
    print(f"Group B: Purchasers = {n_orders_B}, AOV = {aov_B:.2f}, AOV SD = {sd_aov_B:.2f}\n")

    # Welch's t-test for ARPU (using full revenue data)
    ...
    print("----- ARPU Comparison -----")
    print(f"t-statistic: {t_stat_arpu:.3f}")
    print(f"Degrees of freedom: {df_arpu:.1f}")
    print(f"p-value: {p_value_arpu:.3f}")
    print(f"Confidence Level: {confidence_level_arpu:.1f}%")
    print(f"ARPU Lift (Variant vs. Control): {arpu_lift:.2f}%\n")

    # Welch's t-test for AOV (using purchaser revenue data)
    # Only perform the test if there are orders in both groups
    if n_orders_A > 1 and n_orders_B > 1:
        ...
        print("----- AOV Comparison -----")
        print(f"t-statistic: {t_stat_aov:.3f}")
        print(f"Degrees of freedom: {df_aov:.1f}")
        print(f"p-value: {p_value_aov:.3f}")
        print(f"Confidence Level: {confidence_level_aov:.1f}%")
        print(f"AOV Lift (Variant vs. Control): {aov_lift:.2f}%")
    else:
        print("Not enough data to perform AOV comparison for one or both groups.")
